...
var from = require('@dwcore/dws2wrap')
...
function Ddb (createStorage, key, opts) {
  if (!(this instanceof Ddb)) return new Ddb(createStorage, key, opts)
  ...
  this.key = key || null
  this.revelationKey = this.key && crypto.revelationKey(this.key)
  ...
  this._ready = dethunk(open) // TODO: if open fails, do not reopen next time
  ...
  this._waiting = []
  this._selections = []
  this._reserved = thinBit()
  this._synced = null

  this._codec = toCodec(opts.valueEncoding)
  this._sync = low(sync)
  if (!this.thin) this.download({start: 0, end: -1})
...
Ddb.revelationKey = crypto.revelationKey

// TODO: instead of using a getter, update on remote-update/add/remove
Object.defineProperty(Ddb.prototype, 'remoteLength', {
  enumerable: true,
  get: function () {
    var len = 0
...
Ddb.prototype.replicate = function (opts) {
  // Lazy load replication deps
  if (!replicate) replicate = require('./lib/replicate')

  if ((!this._selections.length || this._selections[0].end !== -1) && !this.thin && !(opts && opts.live)) {
    // hack!! proper fix is to refactor ./replicate to *not* clear our non-thin selection
...
Ddb.prototype.ready = function (onready) {
  this._ready(function (err) {
    if (!err) onready()
  })
}

Ddb.prototype.update = function (len, cb) {
...
Ddb.prototype._writeStateReloader = function (cb) {
  var self = this
  return function (err) {
    if (err) return cb(err)
...
Ddb.prototype._open = function (cb) {
  var self = this
  ...
  this._storage.openKey(function (_, key) {
    ...
    self.revelationKey = self.key && crypto.revelationKey(self.key)
    self._storage.open({key: self.key, revelationKey: self.revelationKey}, onopen)
  })
  ...
  function onopen (err, state) {
    ...
    function onsignature (_, sig) {
      ...
      if (!writable && self.writable) return cb(new Error('Ddb is not writable'))
...
Ddb.prototype.download = function (range, cb) {
...
Ddb.prototype.undownload = function (range) {
  if (typeof range === 'number') range = {start: range, end: range + 1}
  if (!range) range = {}
...
Ddb.prototype.digest = function (index) {
  return this.tree.digest(2 * index)
}

Ddb.prototype.proof = function (index, opts, cb) {
...
Ddb.prototype._readyAndProof = function (index, opts, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
...
Ddb.prototype.put = function (index, data, proof, cb) {
  if (!this.opened) return this._readyAndPut(index, data, proof, cb)
  this._putBuffer(index, this._codec.encode(data), proof, null, cb)
}

Ddb.prototype.cancel = function (start, end) {  // TODO: use same argument scheme as download
  if (!end) end = start + 1
...
Ddb.prototype._cancel = function (start, end) {
  var i = 0
...
Ddb.prototype.clear = function (start, end, opts, cb) { // TODO: use same argument scheme as download
...
Ddb.prototype.signature = function (index, cb) {
...
Ddb.prototype.verify = function (index, signature, cb) {
  var self = this
...
Ddb.prototype.rootHashes = function (index, cb) {
  this._getRootsToVerify(index * 2 + 2, {}, [], cb)
}

Ddb.prototype.seek = function (bytes, opts, cb) {
...
Ddb.prototype._seek = function (offset, cb) {
...
Ddb.prototype._readyAndSeek = function (bytes, opts, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
...
Ddb.prototype._getBuffer = function (index, cb) {
  this._storage.getData(index, cb)
}

Ddb.prototype._putBuffer = function (index, data, proof, from, cb) {
...
Ddb.prototype._readyAndPut = function (index, data, proof, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
...
Ddb.prototype._write = function (index, data, nodes, sig, from, cb) {
...
Ddb.prototype._writeAfterHook = function (index, data, nodes, sig, from, cb) {
  var self = this
...
Ddb.prototype._writeDone = function (index, data, nodes, from, cb) {
...
Ddb.prototype._verifyAndWrite = function (index, data, proof, localNodes, trustedNode, from, cb) {
  var visited = []
...
Ddb.prototype._verifyRootsAndWrite = function (index, data, top, proof, nodes, from, cb) {
  var remoteNodes = proof.nodes
...
Ddb.prototype._getRootsToVerify = function (verifiedBy, top, remoteNodes, cb) {
...
Ddb.prototype._announce = function (message, from) {
  for (var i = 0; i < this.peers.length; i++) {
...
Ddb.prototype._readyAndGet = function (index, opts, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
...
Ddb.prototype.getBatch = function (start, end, opts, cb) {
...
Ddb.prototype._getBatch = function (start, end, opts, cb) {
...
Ddb.prototype._readyAndGetBatch = function (start, end, opts, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
...
Ddb.prototype._updatePeers = function () {
  for (var i = 0; i < this.peers.length; i++) this.peers[i].update()
}

Ddb.prototype.createWriteStream = function () {
  var self = this
...
Ddb.prototype.createReadStream = function (opts) {
  if (!opts) opts = {}
...
// TODO: when calling finalize on a live ddb write an END_OF_DDB block (length === 0?)
Ddb.prototype.finalize = function (cb) {
  if (!this.key) {
    this.key = crypto.tree(this._merkle.roots)
    this.revelationKey = crypto.revelationKey(this.key)
  }
  this._storage.key.write(0, this.key, cb)
}

Ddb.prototype.append = function (batch, cb) {
  this._batch(Array.isArray(batch) ? batch : [batch], cb || noop)
}

Ddb.prototype.flush = function (cb) {
  this._batch([], cb)
}

Ddb.prototype.close = function (cb) {
  var self = this
...
Ddb.prototype._appendHook = function (batch, cb) {
  var self = this
...
Ddb.prototype._append = function (batch, cb) {
  if (!this.opened) return this._readyAndAppend(batch, cb)
  if (!this.writable) return cb(new Error('This ddb is not writable. Did you create it?'))
...
Ddb.prototype._readyAndAppend = function (batch, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
    self._append(batch, cb)
...
Ddb.prototype._readyAndCancel = function (start, end) {
  var self = this
  this.ready(function () {
...
Ddb.prototype._pollWaiting = function () {
...
Ddb.prototype._syncBitfield = function (cb) {
  ...
  // Added benefit is that if the program exits while flushing the bitfield the ddb will only get
...
Ddb.prototype._roots = function (index, cb) {
