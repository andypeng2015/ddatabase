...
Storage.prototype.putBitfield = function (offset, data, cb) {
  this.thinBit.write(32 + offset, data, cb)
}
...
Storage.prototype.open = function (opts, cb) {
  ...
  if (!this.thinBit) this.thinBit = this.create('thinBit', opts)
  if (!this.signatures) this.signatures = this.create('signatures', opts)

  var result = {
    thinBit: bufferAlloc(0),
    secretKey: null,
    key: null
  }

  this.thinBit.write(0, header(0, 3328, null), function (err) {
    if (err) return cb(err)
    readAll(self.thinBit, 32, 3328, function (err, data) {
      if (data) result.thinBit = data
      done(err)
    })
  })
