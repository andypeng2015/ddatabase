...
tape('dDatabase Core Tests: deterministic data and tree', function (t) {
  t.plan(10)
  ...
  function run () {
    var st = storage()
    var ddb = ddatabase(st)

    ddb.append(['a', 'b', 'c', 'd', 'e', 'f'], function () {
...
tape('dDatabase Core Tests: deterministic data and tree after replication', function (t) {
  t.plan(10)
  ...
  function run () {
    ...
    ddb.append(['a', 'b', 'c', 'd', 'e', 'f'], function () {
      var st = storage()
      var fork = ddatabase(st, ddb.key)
      var stream = fork.replicate()

      stream.pipe(ddb.replicate()).pipe(stream).on('end', function () {
        t.same(st.data.toBuffer().toString(), 'abcdef')
        t.same(st.tree.toBuffer(), expectedTree)
      })
    })
...
tape('dDatabase Core Tests: deterministic signatures', function (t) {
  t.plan(10)
...
tape('dDatabase Core Tests: deterministic signatures after replication', function (t) {
  t.plan(10)
